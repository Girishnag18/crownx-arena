...
import { Crown, Loader2, Sparkles, Swords, Trophy } from "lucide-react";
...
const Dashboard = () => {
  ...
  const loadProfile = async (userId: string) => {
    ...
    if (data) {
      const loadedProfile = data as unknown as Profile;

      if (prevRankRef.current && prevRankRef.current !== loadedProfile.rank_tier) {
        const ranks = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Crown Master"];
        if (ranks.indexOf(loadedProfile.rank_tier) > ranks.indexOf(prevRankRef.current)) {
          setPromotion({ oldRank: prevRankRef.current, newRank: loadedProfile.rank_tier });
        }
      }

      prevRankRef.current = loadedProfile.rank_tier;
  ...
  const loadRecentGames = async (userId: string) => {
    ...
    const opponentIds = Array.from(
      new Set(
        games
          .map((game) => (game.player_white === userId ? game.player_black : game.player_white))
          .filter(Boolean),
      ),
    ) as string[];
    ...
    if (opponentIds.length > 0) {
      ...
      if (profiles) {
        names = new Map(profiles.map((entry) => [entry.id, entry.username || "Player"]));
      }
    }
  ...
  const loadRecentTournaments = async () => {
    ...
    if (data) {
      setRecentTournamentsList(data as RecentTournamentRow[]);
    }
  };
  ...
  const loadMyRegistrations = async (userId: string) => {
    ...
    if (data) {
      setRegisteredTournamentIds((data as { tournament_id: string }[]).map((entry) => entry.tournament_id));
    }
  };
  ...
  useEffect(() => {
    ...
    const ch1 = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles", filter: `id=eq.${user.id}` }, () => loadProfile(user.id))
      .subscribe();
    ...
    const ch4 = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "recent_tournaments" }, loadRecentTournaments)
      .subscribe();
  ...
  const createTournament = async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select("id, name, prize_pool, max_players, status, starts_at")
      .single();
    ...
    if (data) {
      setTournaments((prev) => [{ ...(data as Tournament), registration_count: [{ count: 0 }] }, ...prev]);
    }
  ...
  const cancelTournament = async (tournament: Tournament) => {
    ...
    for (const registration of registrations || []) {
      const { data: profileData } = await supabase.from("profiles").select("wallet_crowns").eq("id", registration.player_id).single();
      await supabase
        .from("profiles")
        .update({ wallet_crowns: Number(profileData?.wallet_crowns || 0) + 2 })
        .eq("id", registration.player_id);
      await supabase.from("wallet_transactions").insert({ player_id: registration.player_id, amount: 2, txn_type: "tournament_refund" });
      await (supabase as any).from("player_notifications").insert({
        user_id: registration.player_id,
        title: "Tournament cancelled",
        message: `"${tournament.name}" was cancelled. Refund issued.`,
  ...
  const activeTournaments = useMemo(
    () => tournaments.filter((tournament) => tournament.status !== "completed" && tournament.status !== "cancelled"),
    [tournaments],
  );
    ...
    <div className="page-container">
      ...
      <PullToRefresh onRefresh={handlePullRefresh}>
        <div className="page-content">
          <motion.div initial="hidden" animate="show" variants={stagger} className="page-stack">
            <motion.div variants={fadeUp}>
              <PageHeader
                badge="Arena hub"
                badgeIcon={Trophy}
                title={`Welcome back, ${displayName}`}
                description="Track your live rank, jump into matches faster, and keep the main arena tools aligned in one clean dashboard."
                meta={[
                  { icon: Sparkles, label: `${rank} rank` },
                  { icon: Crown, label: `${profile?.wallet_crowns || 0} crowns` },
                  { icon: Swords, label: `${activeTournaments.length} active tournaments` },
                ]}
              />
            </motion.div>
                ...
                <motion.div variants={fadeUp}>
                  <TournamentsCard
                    ...
                    createFormProps={{
                      ...
                      setName: setNewTournamentName,
                      prizePool: newPrizePool,
                      setPrizePool: setNewPrizePool,
                      maxPlayers: newMaxRegistrations,
                      setMaxPlayers: setNewMaxRegistrations,
                      startsAt: newStartsAt,
                      setStartsAt: setNewStartsAt,
                      type: newTournamentType,
                      setType: setNewTournamentType,
